...
class Middlebury(Dataset):
    ...
    def __getitem__(self, index, called=0):
        ...
        try:
            ...
            return {'img_rgb': image, 'img_gt': depth_map, 'img_lr': source, 'img_mask': mask_hr, 'mask_lr': mask_lr,
                    'img_name': im_index, 'lr_up': y_bicubic}
        except:
            return self.__getitem__(index, called=called + 1)

    def __len__(self):
        return len(self.deterministic_map if self.crop_deterministic else self.data * 100)
