...
class NYU(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.crop_deterministic:
            ...
            index = index % len(self.split_idc)
        ...
        return {
            'img_rgb': outputs[0], 'img_gt': outputs[1], 'img_lr': source,
                'img_mask': mask_hr, 'mask_lr': mask_lr, 'lr_up': y_bicubic
        }

    def __len__(self):
        if self.crop_deterministic:
            return len(self.split_idc) * (self.H // self.crop_size[0]) * (self.W // self.crop_size[1])
        return len(self.split_idc) * 100
