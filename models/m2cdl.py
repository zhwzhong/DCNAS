...
import models.mmir.basicblock as B
import torch.nn.functional as F
import numpy as np
from math import ceil
from models.mmir.utils import *
...
class DCDicL(nn.Module):
    ...
    def forward(self, samples):
        x, y = samples['lr_up'], rgb_to_grayscale(samples['img_rgb'])
        device = torch.cuda.current_device()
        sigma = torch.FloatTensor([1.0 / self.down_scale]).unsqueeze(1).unsqueeze(1).unsqueeze(1)
        sigmay = sigma
        sigma = sigma.to(device)
        sigmay = sigmay.to(device)
        # padding
        h, w = y.size()[-2:]
...
def make_model(args): return DCDicL(in_nc=args.in_channels, out_nc=args.in_channels, scale=args.scale)


# if __name__ == '__main__':
#     from config import args
#     from calc_complexity import calculate_latency, calculate_parameters
#     model = DCDicL(in_nc=args.in_channels, out_nc=args.in_channels, scale=args.scale).cuda()
#     cd, cg, h, w = 1, 3, 480, 480
#     samples = {
...
#     }
#     print(calculate_parameters(model, h, w))
#     print(calculate_latency(model, cd, cg, h, w, torch.device('cuda')))
...
#     end_time = start.elapsed_time(end) / 10
#     print(end_time / 10)
