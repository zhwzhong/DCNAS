...
class DMSG(nn.Module):
    def __init__(self, args):
        ...
        self.gaussian_rgb = torch_gaussian(channels=1, kernel_size=15, sigma=5)
        self.gaussian_dep = torch_gaussian(channels=args.in_channels, kernel_size=15, sigma=5)
        ...
        self.feature_extra = ConvBNReLU2D(args.in_channels, 64, kernel_size=5, stride=1, padding=2, act='PReLU')
        ...
        self.branchMain.append(ConvBNReLU2D(32, args.in_channels, 5, stride=1, padding=2, act='PReLU'))

    def forward(self, samples):
        lr, rgb_img, lr_up = samples['img_lr'], samples['img_rgb'], samples['lr_up']
        rgb_img = rgb_to_grayscale(rgb_img, 1)
        h_Yh = rgb_img - self.gaussian_rgb(rgb_img)
        ...
        outputs_Main = [self.feature_extra(lr - self.gaussian_dep(lr))]
        ...
        return {'img_out': outputs_Main[-1] + self.gaussian_dep(lr_up)}
