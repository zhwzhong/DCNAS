...
class FDKN(nn.Module):
    def __init__(self, kernel_size=3, filter_size=15, residual=True, scale=4):
    ...
    def forward(self, samples):
        ...
        if depth.shape[-1] < image.shape[-1]:
            depth = F.interpolate(depth, scale_factor=self.scale, mode='bilinear', align_corners=False)
