...
def grid_generator(k, r, n, device):
    """grid_generator
    Parameters
    ---------
    f : filter_size, int
    ...
    return grid.unsqueeze(0).repeat(n, 1, 1, 1).to(device)
...
class Kernel_DKN(nn.Module):
    ...
    def forward(self, x):
        ...
        weight = self.sigmoid(self.conv_weight(x))
...
class DKN(nn.Module):
    def __init__(self, args, kernel_size=3, filter_size=15, residual=True):
        ...
        self.args = args
        self.ImageKernel = Kernel_DKN(input_channel=args.guidance_channels, kernel_size=kernel_size)
    ...
    def forward(self, samples):
        depth, image = samples['lr_up'], samples['img_rgb']
        ...
        # (b*hw, r, r, 2)
        grid = grid_generator(k, r, b * hw, depth.device)
        ...
        depth_sampled = F.grid_sample(depth_col.contiguous(), coord, align_corners=True).contiguous().view(b * hw, 1, -1).contiguous()
...
# model = DKN(args=args).cuda()
# arr = torch.randn(1, 1, 128, 128).cuda()
# print(model(arr, arr.repeat(1, 3, 1, 1)))