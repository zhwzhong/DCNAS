...
class FeatureInitialization(nn.Module):
    ...
    def __init__(self, num_features, scale, in_channels=1, guidance_channel=3):
        super(FeatureInitialization, self).__init__()

        self.rgb_shuffle = invPixelShuffle(ratio=scale)

        self.depth_in = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=num_features, kernel_size=3, stride=1, padding=1),
            nn.PReLU()
        )
...
class AHMF(nn.Module):
    def __init__(self, scale=4, act='PReLU', in_channels=1, guidance_channels=3):
        super(AHMF, self).__init__()

        self.head = FeatureInitialization(num_features=64, scale=scale, in_channels=in_channels, guidance_channel=guidance_channels)
        ...
        self.up_conv = nn.Sequential(
            ...
            *UpSampler(scale=scale, n_feats=64),
            ConvBNReLU2D(in_channels=64, out_channels=in_channels, kernel_size=3, padding=1, norm=None)
        )
...
def make_model(args): return AHMF(
    args.scale, act=args.act, in_channels=args.in_channels, guidance_channels=args.guidance_channels)
