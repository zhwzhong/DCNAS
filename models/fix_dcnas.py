...
class DownSample1(nn.Module):
    ...
    def forward(self, dep, rgb):
        return self.dep_layers(dep), rgb
...
class StaticModel(nn.Module):
    def __init__(self, args, arch=None):
        ...
        # self.FeatureInitialization = FeatureInitialization(args.num_features, args.act, args.bias, args.norm)

        num_features = [args.num_features * pow(2, i) for i in range(args.num_stages)]

        self.num_rgb_blocks = args.num_stages

        for op in list(arch['fuse_op']):
            if op == (args.num_fuse_ops - 1) or op == 0:
                self.num_rgb_blocks -= 1
            else:
                break
        ...
        for stage in range(args.num_stages):
            ...
            if stage != args.num_stages - 1:
                if stage < (self.num_rgb_blocks - 1):
                    self.down_sample.append(DownSample(2, num_features=num_features[stage], bias=args.bias, keep_dim=False))
                else:
                    self.down_sample.append(DownSample1(2, num_features=num_features[stage], bias=args.bias, keep_dim=False))
...
# def make_model(args, arch): return DCNAS(args, arch)

# from config import args as arg
# from utils import get_random_architecture
# net = DCNAS(arg, get_random_architecture(arg.num_blocks, arg.num_stages, arg.num_base_ops, arg.num_fuse_ops))
# d = torch.randn(1, 1, 640, 480)
# r = torch.randn(1, 3, 640, 480)
# print(net.up_sample)
# print(net(dep=d, rgb=r)['img_out'].size())
